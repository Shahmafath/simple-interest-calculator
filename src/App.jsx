...
function App() {
  ...
  const validateUserInputs = (e) => {
    // destructuring because it is an object
    const {name, value} = e.target
    // here, typeof value is string so we can use regular expression
    // in regular expression, we start with ^ and end with $
    // here the regular expression we use is a decimal
    // !! is a boolean where it returns true or false 
  ...
  return (
    <div style={{ width: '100%', height: '100vh' }} className='d-flex justify-content-center align-items-center bg-dark'>
      <div style={{ width: '600px' }} className='bg-light p-5'>
        <h3>Simple Interest Calculator</h3>
        <p>Calculate your simple interest easily</p>
        <div style={{ width: '100%', height: '150px' }} className='d-flex justify-content-center align-items-center bg-warning mt-5 text-light shadow flex-column'>
        ...
        <form className="mt-5" onSubmit={handleCalculate}>
          ...
          {!validYear&& <div className="mb-3 text-danger fw-bolder"> Invalid Year Amount</div>}

          <Stack spacing={2} direction={'row'}>
            <Button disabled={validPrinciple && validRate && validYear?false:true} type='submit' style={{height: '70px', width: '50%'}} className='bg-dark' variant="contained">CALCULATE</Button>

            <Button style={{height: '70px', width: '50%'}}
            className='text-dark' variant="outlined" onClick={handleReset}>RESET</Button>
          </Stack>
        </form>
      </div>
    </div>
